...
export function monthsBetween(begDate: Date, endDate: Date): Date[] {
  let t = []
  let tDate = begDate
  do {
    t.push({ start: tDate, end:  eoMonth(tDate,0) <= endDate ? eoMonth(tDate,0) : endDate })
...
// goal here is do eoMonth but for multiple given intervals. Doesn't work yet.
export function dateSpread(begDate: Date, endDate: Date, interval: string): Date[] {
  interval = interval.toLocaleLowerCase()
  let idx = {
    year:   { adj: (interval == 'year')     ? 1 : 0, toZero: false},
    month:  { adj: (interval == 'month')    ? 1 : 0, toZero: ['year'].indexOf(interval) > -1},
    day:    { adj: (interval == 'day')      ? 1 : 0, toZero: ['year','month'].indexOf(interval) > -1},
    hour:   { adj: (interval == 'hour')     ? 1 : 0, toZero: ['year','month','day'].indexOf(interval) > -1},
    minute: { adj: (interval == 'minute')   ? 1 : 0, toZero: ['year','month','day','hour'].indexOf(interval) > -1}
  }
